...
class ScoreLLM(AgentBase):
    ...
    def run_agent(self, **kwargs):
        ...
        # agent_s analyse
        prompt = evaluate_quality_prompt(
            ScoreAnswerDatasForm.model_json_schema(), question, answer
        )
        sys_prompt = "You are an expert evaluator of health decision support systems."
        output, total_tokens = self.brain(
            sys_prompt=sys_prompt, user_prompt=prompt, formating=ScoreAnswerDatasForm
        )
        tokens_used += total_tokens
        output_dict = {
            "trustworthiness": output.trustworthiness,
...
def get_evidence_string(data, max_width=10000, is_show_response=False):
    ...
    lines.append(
        "The user question "
        + agent_tree_dict["user_question"]
        + ".Decompose into the following multiple intents:"
    )
...
if __name__ == "__main__":
    ...
    intent_answer_s = json_read_to_dict("./mult_dim_trend_answer_s.json")
    score_s = json_read_to_dict('./mult_dim_trend_scores.json')
    ...
    for intent_answer in intent_answer_s[last_role_id: ]:
        ...
        role_score = {"role_id": role_id, "score_dict_s": []}
        for i in range(len(questions)):
            question = questions[i]
            mult_agents_answer = mult_agents_answers[i]
            llm_baseline_answer = llm_baseline_answers[i]
            mult_evidence_path = "./mult_dim_trend/role_{}_question_{}.json".format(
                role_id, i + 1
            )
            ...
            mult_agent_score, agent_tokens_used = score_llm.run_agent(
                question=question, answer=mult_agents_answer
            )
            llm_baseline_score, llm_tokens_used = score_llm.run_agent(
                question=question, answer=llm_baseline_answer
            )
        ...
        dict_save_to_json(score_s, "./mult_dim_trend_scores.json")
    dict_save_to_json(score_s, "./mult_dim_trend_scores.json")
    print(score_s)
